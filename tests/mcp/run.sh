...
# entur-kb MCP + the markdown docs it indexes (not to local Read/Grep).
#
# Usage:
...
SYSTEM_PROMPT='You are evaluating the entur/ai knowledge base via the entur-kb MCP server.
You may ONLY use the mcp__entur-kb__search_entur_kb tool. Do not call any other tool.
...
ALLOWED_TOOLS='mcp__entur-kb__search_entur_kb'
