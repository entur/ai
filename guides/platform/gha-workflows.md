...
Split CI/CD into focused workflow files generated by the [`setup-cicd-workflows`](../../skills/setup-cicd-workflows/SKILL.md) skill. Use image promotion: PRs build and push images, merges resolve the PR-built image via git tag and deploy it.
...
Replace the Java test job in `ci.yml` with:
...
Use the [`setup-cicd-workflows`](../../skills/setup-cicd-workflows/SKILL.md) skill to generate the focused workflow files. Do not maintain a hand-written workflow inventory here; duplicated file lists drift from the generated templates.

### ci.yml (Reusable Build Workflow)

Called by `build.yml` (PRs) and `cd.yml` (workflow_dispatch). Helm linting runs inside this workflow so charts are validated on every build, not only when `helm/**` files change.
...
### build.yml (PR Build Trigger)

Runs CI on every pull request. The built image is pushed so it can be resolved by `cd.yml` after merge.
...
### dependabot-pr.yml (Dependabot Approval Gate)
...
16. Dependabot PRs need human approval before CI runs with secrets (`dependabot-pr.yml`)
