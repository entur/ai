...
description: >
  ...
  project, generates an AGENTS.md that references Entur-wide standards (read by
  Claude Code, Codex, and other AGENTS.md-compatible agents), configures
  Claude Code-specific tool permissions and an optional formatting hook,
  recommends Entur skills, and updates .gitignore. Codex tool permissions are
  configured per-user outside the repository and are not generated by this
  skill. Use this skill when the user says "set up Claude Code", "set up agent
  config", "create AGENTS.md", "auto-setup", "onboard this repo for AI
  agents", or wants agent instructions added to an existing repository.
---

# Entur Agent Config Setup

Configure an existing Entur repository so AI agents work with it safely and follow Entur platform standards. Generate the minimum configuration that points agents at the Entur-wide standards -- do **not** copy standards into the repository.

**Scope by agent**: `AGENTS.md` (Steps 1-2) is shared -- Claude Code, Codex, and other AGENTS.md-reading agents all use it. Tool permissions and the formatting hook (Steps 3-4) are **Claude Code-specific**, written to `.claude/settings.json`. Codex has no equivalent repository-committed permissions file -- its sandbox mode and approval policy are configured per-user in `~/.codex/config.toml` or via CLI flags (`--sandbox`, `--ask-for-approval`), outside this repository and outside what this skill can generate. When a user asks this skill to "set up Codex safely," generate `AGENTS.md` and tell them explicitly that Codex's own sandbox/approval settings are a separate, per-user configuration step this skill does not perform -- do not report Claude-only settings as having configured Codex.
...
Write tool permissions to `.claude/settings.json`. This step is Claude Code-specific -- see the Scope by agent note above. Merge with existing content -- never replace the file. Allow the build, test, and lint commands detected in Step 1; deny mutations of cloud infrastructure and reads of secret material.
...
## Step 4: Add a Claude Code Formatting Hook (Conditional)
...
2. Claude Code permissions allowed and denied
3. Skills recommended and the marketplace install command
4. If the user asked about Codex: an explicit reminder that Codex sandbox/approval settings are a separate, per-user `~/.codex/config.toml` step this skill did not perform
5. Next steps: review the diff, commit with a Conventional Commit (`chore: add AI agent configuration`), open a PR
